...
- `{ action: "memory-tool", password, name, arguments }`：`name` 只认 `memory_search` / `memory_recall`，转到 ember `/internal/memory/search|recall`；参数错、未配置、超时、404 都回 200 + `{ ok:false, error }`，成功回 `{ ok:true, result }`。
