...
folderDetailList.addEventListener("click", (event) => {
  ...
  if (action === "fd-prompt") {
    ...
    createFolderInteractive().then((folder) => {
      if (folder) {
        moveConversationToFolder(conversationId, folder.id);
        if (conversationById(conversationId)?.folderId === folder.id) folderDetailPickerId = null;
      }
      rerender();
    });
...
folderNew.addEventListener("click", async () => {
  const folder = await createFolderInteractive();
